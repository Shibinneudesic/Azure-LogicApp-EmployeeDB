...
Write-Host ""
Write-Host "[Step 3] Creating storage queues in Azurite..." -ForegroundColor Yellow
Write-Host ""
...
foreach ($queueName in $queues) {
    ...
    try {
        $response = Invoke-WebRequest -Uri $uri -Method PUT -Headers @{"x-ms-version" = "2021-08-06"} -ErrorAction SilentlyContinue
...
Write-Host ""
Write-Host "[Step 4] Creating test file directory..." -ForegroundColor Yellow
Write-Host ""
